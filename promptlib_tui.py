...
try:
    ...
    npyscreen.wrapper_basic(lambda: npyscreen.notify_confirm(
        f"Could not import promptlib.py in the current directory.\n\nError: {e}", 
        title="FATAL: ImportError", wide=True))
    sys.exit(1)
...
if not get_category_choices():
    npyscreen.wrapper_basic(lambda: npyscreen.notify_confirm(
        ...
        title="FATAL: No Categories", wide=True))
    sys.exit(1)
...
class PromptBatchForm(npyscreen.ActionForm):
    def create(self):
        ...
        self.number = self.add(
            npyscreen.TitleText,
            name="Prompt Count per Category",
            value="5"
        )
        self.config = self.add(
            npyscreen.TitleFilenameCombo,
            name="Optional Config Path",
            value=""
        )
    ...
    def on_ok(self):
        try:
            ...
            if not chosen_indices or not isinstance(chosen_indices, list):
                npyscreen.notify_confirm("Please select at least one category.", title="Input Error", wide=True)
                return
            ...
            if number < 1 or number > 1000:
                npyscreen.notify_confirm("Prompt count must be between 1 and 1000.", title="Input Error", wide=True)
                return
            if not is_valid_config(config_path):
                npyscreen.notify_confirm("Config path is invalid or file does not exist.", title="Input Error", wide=True)
                return
        except Exception as e:
            npyscreen.notify_confirm(f"Input Error: {e}", title="Input Error", wide=True)
            return
...
class PromptPreviewForm(npyscreen.ActionForm):
    def create(self):
        ...
        self.preview_box = self.add(npyscreen.BoxTitle, name="Preview Prompts", max_height=16, rely=2)
        self.add(npyscreen.FixedText, value="Press OK to save these prompts and continue, or Cancel to skip this category.", editable=False, relx=2, rely=19)
    ...
    def beforeEditing(self):
        ...
        self.title.value = f"Category: {self.cur_category} ({self.category_idx+1} of {len(cats)})"
        ...
        try:
            ...
            self.prompts = [promptlib.generate_prompt(template, slotset) for _ in range(self.parentApp.prompt_count)]
        except Exception as e:
            self.prompts = [f"[ERROR] {e}"]
        # Show preview
        self.preview_box.values = [f"{idx+1}. {p}" for idx, p in enumerate(self.prompts)]
    ...
    def on_ok(self):
        ...
        npyscreen.notify_confirm(f"Prompts saved to: {outfile}\n\n(Next category will be shown, if any remain)", title="Saved", wide=True)
...
class ResultForm(npyscreen.FormBaseNew):
    def create(self):
        self.message = self.add(npyscreen.FixedText, value="Prompt generation complete.", editable=False, relx=2)
        self.add(npyscreen.FixedText, value="Audit logs in prompt_logs/prompt_audit.log", editable=False, relx=2, rely=3)
        self.add(npyscreen.FixedText, value="Press ENTER or ESC to exit.", editable=False, relx=2, rely=5)
...
def safe_cli_menu():
    ...
    for _ in range(n):
        print(promptlib.generate_prompt(promptlib.TEMPLATES[cats[i]], promptlib.SLOTS[cats[i]]))
